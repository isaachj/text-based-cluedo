...
public class CluedoGUI extends GUI {
	...
	private Suggestion getAccusation(){
		RoomCard room = null;
		WeaponCard weapon = null;
		CharacterCard character = null;
		...
		for (int i = 0; i < options.length; i++){
			options[i] = rooms.get(i).getName();
		}

		Object selected = JOptionPane.showInputDialog(null, "Choose a room to suggest", "Selection", JOptionPane.DEFAULT_OPTION, null, options, "0");
		if ( selected != null ){ // null if the user cancels.
			String selectedString = selected.toString();

			for (Card c : deck){
				if (c instanceof RoomCard && selectedString.equals(c.getName())){
					room = (RoomCard) c;
				}
			}
		}
		...
		selected = JOptionPane.showInputDialog(null, "Choose a weapon to suggest", "Selection", JOptionPane.DEFAULT_OPTION, null, options, "0");
		if ( selected != null ){ // null if the user cancels.
			String selectedString = selected.toString();
